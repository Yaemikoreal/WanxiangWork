...
class decide(object):
    def __init__(self):
        ...
        self.headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36 Edg/118.0.2088.76'}
    ...
    def get_sql_menus(self,key,menuname):
        ...
        try:
            ...
            sql = """
                    SELECT [key], [value]
                    FROM [FddLaw5.0].[dbo].[menus]
                    WHERE [key] LIKE ? AND [menuname] LIKE ? AND parentid LIKE ?
                """

            # 执行 SQL 查询
            cursor.execute(sql, (f'{key}%', f'%{menuname}%', f'{key}%'))
    ...
    def ask(self,page):
        ...
        else:page_url=self.main_url.replace('index.html',f'index_{page}.html')
        print(page_url)
        ...
        for one_data in lilist:
            ...
            pagedate = self.split(str(one_data.find('span',class_='time').text.strip()).replace('成文日期 ：','').replace('年','.').replace('月','.').replace('日','.'))
            ...
            if title:
                if 'http' in href:
                    ...
                    href='https://czj.cq.gov.cn/zwgk_268/zfxxgkml/zcwj/gfxwj'+one_data.find('a').get('href')[1:]#法规链接
                    print(href)
                    # continue
                ddd = '重庆'
                sss = chl_esquc(title, '标题')
                result_renshi = any(data in title for data in self.gjc)#人事任免信息
                if (sss or '公告' in title or '通报' in title or '通告' in title or '公示' in title or '政策解读' in title or '答记者问' in title or '资质审批名单' in title or title[-2:] == '工作' or title[-3:] == '培训会' or '征求意见稿' in title) and '...'not in title:
                    pass
                else:
                    ...
                    if (lar_esquc(title, '标题') or a) and title[-3:] != '...':
                        ...
                        if (lar_esquc(Issued_Number,'发文字号') or b) and len(Issued_Number.strip())>2:
                            ...
                            if Issued_Number == '':
                                ...
                                # print(Issued_Number)
                                if len(Issued_Number) > 30 or len(Issued_Number) < 3 or '公示编号' in Issued_Number: Issued_Number = ''
                                if Issued_Number == '':
                                    try:
                                        Issued_Number = \
