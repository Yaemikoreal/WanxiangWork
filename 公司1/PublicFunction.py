...
import logging
from 公司1.QueryTitle import main_panduan
from 公司1.分类 import get_data_from_api, key_word
import re
import pyodbc
import requests
...
def annex_get(url, save_path, headers, save_path_real='重庆市发改委其他文件'):
    """
    本函数用于附件内容获取
    :param save_path_real:
    :param url:
    :param save_path: 保存名
    :param headers: 请求头
    :return:
    """
    save_path_real = f'E:/下载附件/{save_path_real}/' + save_path
    exist = os.path.exists(save_path_real)
    if exist:
...
def filter(result_lt):
    ...
    for result in result_lt:
        ...
        if main_panduan(title_a=title, fm='831', issued_date=issued_date,issued_number=issued_number,):
...
def soup_cal(soup_ture):
    ...
    #                 # 将style值分割成多个样式
    #                 styles = style_value.split(';')
    #                 if "text-align:left" in styles or "text-align: left" in styles:
    #                     del tag['style']
    #                     continue
    #             else:
    #                 del tag['style']
    #                 continue
    # if not soup_ture.find_all(True):
    #     style = soup_ture.get('style')
    #     data_index = soup_ture.get('data-index')
    #     if data_index:
    #         del soup_ture['data-index']
    #     if style:
    #         if determine(style):
    #             if 'text-align' in style:
    #                 # 获取当前元素的style属性值
    #                 style_value = soup_ture['style']
    #                 # 将style值分割成多个样式
    #                 styles = style_value.split(';')
    #                 new_styles = []
    #                 for it in styles:
    ...
    #                 # 重新组合样式
    #                 if new_styles:
    #                     soup_ture['style'] = '; '.join(new_styles)
    #                 else:
    #                     del soup_ture['style']
    #             else:
    #                 del soup_ture['style']
    ...
    not_lt = {"text-align:right", "text-align:center", "text-align: right", 'text-align: center', 'id'}

    def process_style(tag):
        style = tag.get('style')
        if style:
            styles = [s.strip() for s in style.split(';') if s.strip()]
            new_styles = [s for s in styles if s in not_lt]
...
def remove_nbsp(soup, is_add_right=True):
    ...
    for it in soup.find_all('p'):
        ...
        if "抄送" in tag_text:
            it.decompose()
    # 删除具有特定 ID 的 div 标签
    div_to_remove = soup.find('div', id='a1')
    if div_to_remove:
...
def tupiangeshi(Description):
    ...
    return Description
def downfile_img(header, base_url, soup,content,):
    """
    下载文件
    :param header: 请求头部
    :param cookie: Cookie信息
    :param base_url: 基础URL，用于拼接相对路径
    :param soup: 解析后的HTML内容
    :param shouludate: 收录日期
    :return: 更新后的HTML内容
    """
    folder_path = r"E:\下载附件\img"
    ...
    try:
        ...
        if img:
            for img_tag in soup.find_all('img'):  # 找到所有的<img>标签
                src = img_tag.get('src')
                if not src:
                    continue

                image_format = re.findall(
                    r'\.(jpeg|jpg|jpe|jif|jfi|jfif|jpx|jpm|mj2|png|gif|bmp|tiff|tif|raw|cr2|nef|orf|sr2|webp|psd|ai|svg|heif|heic)$',
                    src)
                if not image_format:
                    continue
                name = os.path.basename(src)
                url = src if 'http' in src else base_url + src.replace('../', '/').replace('./', '/')
                ...
                if response.status_code == 200:
                    ...
                    new_path = rf'/datafolder/新收录/{datetime.now().strftime("%Y%m")}/{name}'
