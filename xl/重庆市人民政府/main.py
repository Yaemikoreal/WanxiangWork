...
from xl.重庆市人民政府.clear_content import  main_test as main_clean_cal
import hashlib
        ...
        def fetch_url(self,url):
            ...
            for it in range(cl_num):
                try:
                    ...
                    data=data_json['data']['middle']['list']
                    date_time_format = "%Y-%m-%d %H:%M:%S"
                    for item in data[3:]:
                        tag= item['title'].replace('<em>','').replace('</em>','')[-6:]
                        if tag=="全体会议决议":
                            title = item['title'].replace('<em>', '').replace('</em>', '')
                            href = item['url']
                            content = item['content']
                            fabushijiann=item['time']
                            # 解析日期时间字符串
                            date_time_obj = datetime.strptime(fabushijiann, date_time_format)
                            # 提取日期部分
                            date_only = date_time_obj.date()
                            # 将日期部分转换回字符串
                            fabushijian= date_only.strftime("%Y-%m-%d")

                            results.append({'title': title, 'href': href, "content": content,"fabushijian":fabushijian})
                        else:
                            if "（" in item['title'].replace('<em>','').replace('</em>',''):
                                title= item['title'].replace('<em>', '').replace('</em>', '')
                                href = item['url']
                                content = item['content']
                                fabushijiann = item['time']
                                ...
                                fabushijian = date_only.strftime("%Y-%m-%d")
                                results.append(
                                    {'title': title, 'href': href, "content": content, "fabushijian": fabushijian})
                            else:
                                _log.info("不包含全体会议决议")
            ...
            return None
#'中国共产党重庆市第五届委员会第八次<em>全体会议</em><em>决议</em> （2020年4月15日中国共产党重庆市第五届委员会第八次<em>全体会议</em>通过）'
...
def main_test():
    data_dt = {
        "start_url":"https://wap.cq.gov.cn/irs/front/search",# 访问路径
        "write_table_name": '中共重庆市收录',  # 写入数据库表名
        # 'read_pages_start': 0,  # 读取页码起始数(调试用)
        # "read_pages_num": 9,  # 读取页码总数
        "save_path_real": '重庆市人民政府',  # 附件存放路径的文件名
        "lasy_department": '重庆市人民政府',  # 在函数返回为空的时候指定发布部门
