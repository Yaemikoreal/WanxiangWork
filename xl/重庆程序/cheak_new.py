...
class Judgment:
    ...
    def filter(self,result_lt):
        ...
        """
        new_result_lt = []
        for index, result in result_lt.iterrows():
            title = result['法规标题']
            issued_date = result['发布日期']
            if not issued_date:
                issued_date = None
            try:
                # 尝试重试三次
                for attempt in range(3):
                    if main_panduan(title_a=title, fm='831', issued_date=issued_date):
                        ...
                        _log.info(f"es库没有这条数据------------------需要收录： {title}")
                        break  # 法规不存在，跳出循环
            except Exception as e:
    ...
    def calculate(self):
        ...
        if self.data_df.empty:
            ...
            new_result_lt = self.filter(self.data_df)
            if new_result_lt:
                print("数据库中的数据，es库中没有",self.data_df)
                ...
                if not self.write_table:
                    ...
                    self.pf.to_mysql(filtered_df, write_name_real + '二次修改')
                    _log.info(f" 已经写入{write_name_real}二次修改表!!!")
                return filtered_df
