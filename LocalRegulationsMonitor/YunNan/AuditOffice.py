...
class AuditOffice:
    ...
    def title_filter(self, title):
        ...
        # 排除关键字
        excluded_keywords = {
            '视频解读', '邀请函', '图解', '一图读懂', '专家访谈', '“象”解云南',
            '动画解读', '一图看懂', '图文', '视频', '音频', '会议通知', '评估', '一图速览'}
    ...
    def annex_get_all(self, result_dt, full_text, any_title, any_url):
        ...
        :return:
        """
        file_path = fr"E:/JXdata/省级地区附件下载/云南省数据收录/云南省审计厅/"
        # 检测路径是否存在
        ...
        for test in full_text.find_all('a', href=re.compile(
            ...
            if href_c and "gif" not in filename:
                ...
                try:
                    if not self.public_down(href_c, file_path + filename):
                        self.public_down(self.center_url + src_c, file_path + filename)
        ...
        if 'img' in str(full_text):
            for img in full_text.find_all('img'):
                ...
                if src_i and ".gif" not in src_i:
                    ...
                    src_i_real = self.title_url + src_i
                    try:
                        if not self.public_down(src_i_real, file_path + file_name):
                            self.public_down(self.center_url + src_i, file_path + file_name)
                        fujian.append({"Title": any_title, "SavePath": ysrca, "Url": src_i})
                        time.sleep(random.uniform(2, 4))
                    except Exception as e:
                        _log.error(f"Error downloading {src_i}: {e}")
                        img.attrs = {'href': ysrca}
                        fujian.append({"Title": any_title, "SavePath": ysrca, "Url": any_url})
    ...
    def full_cal(self, any_soup, any_title, any_url):
        ...
        if not self.check_elasticsearch_existence(any_title, 'lar', result_dt.get("文号")):
            ...
            # 全文以及格式处理
            full_text = any_soup.find(['div'], attrs={"class": ["show-detail"]})
...
if __name__ == '__main__':
    data_dt = {
        "start_url": 'http://sjt.gxzf.gov.cn/zwgk/fdzdgknr/zcfg/sjtwj/qtwj/',  # 访问路径
        "lasy_department": '云南省审计厅',  # 在函数返回为空的时候指定发布部门
        "category": ""
    }
    main(data_dt)
