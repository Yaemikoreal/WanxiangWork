...
def main_test():
    data_dt = {
        "start_url": 'https://mzj.cq.gov.cn/zwgk_218/zfxxgkml/zcwj_166256/qtwj_166259/',  # 访问路径
        "write_table_name": '重庆市其他文件',  # 写入表名
        'read_pages_start': 32,  # 读取页码起始数(调试用)
        "read_pages_num": 37,  # 读取页码总数
        "save_path_real": '重庆市其他文件',  # 附件存放路径,
        "lasy_department": '重庆市民政局',  # 在函数返回为空的时候指定发布部门
