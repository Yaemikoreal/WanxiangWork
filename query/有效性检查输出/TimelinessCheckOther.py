...
class TimelinessCheck:
    ...
    def match_validity_period(self, tag_text):
        ...
        if match_years:
            ...
            return {'年': int(match_years_r.group(1))}
        else:
            return None
    ...
    def choose_date(self, row, expiring_date_r, expiring_date, row_date, last_validity_tag):
        for it in [expiring_date, expiring_date_r]:
            if it == row_date:
                _log.info(f"计算的失效日期为:{it}, 原填写失效日期为:{row_date}")
                row_status = '0'
                return row_status, row
            else:
                if not row_date:
                    _log.info(f"计算的失效日期为:{it}, 原填写失效日期为:{row_date}")
                    _log.info(f"匹配到的文本为: {last_validity_tag}")
                    row_status = '10'
                    row['失效日期'] = it
                    return row_status, row
    ...
    def calculate_row(self, row):
        ...
        # 假设full_text_soup是一个BeautifulSoup对象
        tags_with_validity = [tag for tag in full_text_soup.stripped_strings if '有效期' in str(tag)]
        # 获取最后一个包含“有效期”的标签文本
        last_validity_tag = tags_with_validity[-1] if tags_with_validity else None
        if last_validity_tag:
            if "有效期至" not in last_validity_tag:
                # 处理没有给出具体日期的情况
                expiring_time = self.match_validity_period(last_validity_tag)
                if expiring_time:
                    # 分别算出实施日期和发布日期计算得出的最终失效时间
                    ...
                    row_status, row = self.choose_date(row, expiring_date_r, expiring_date, row_date, last_validity_tag)
    ...
    def filter_data_df(self, data_df):
        ...
        data_df_1 = data_df[data_df['状态'].isna()]
        # 获取行数

        num_rows = data_df_1.shape[0]
        # data_df['状态'] = np.nan
